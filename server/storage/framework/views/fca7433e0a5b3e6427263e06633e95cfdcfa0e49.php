<script>
    customRequestData["server_id"] = '<?php echo e(server()->id); ?>';
    ...
    function server_extension(){
        ...
        if(items.length === 0){
            showSwal("<?php echo e(__('Lütfen önce seçim yapınız.')); ?>",'error');
            return false;
        }
        ...
        request('<?php echo e(route('server_extension')); ?>', data, function (response) {
            Swal.close();
            reload();
        }, function(response){
    ...
    function errorSwal(){
        showSwal('<?php echo e(__("Ayarlarınız doğrulanamadı!")); ?>','error',2000);
    }
    ...
    function checkStatus(id) {
        ...
        request('<?php echo e(route('server_check')); ?>', data, function (response) {
    ...
    <?php if($installed_extensions->count() > 0): ?>
        <?php $__currentLoopData = $installed_extensions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        setInterval(function () {
            checkStatus('<?php echo e($service->id); ?>');
        }, 3000);
    ...
    function stats(noSpinner = false) {
        ...
        var time = "<?php echo e(\Carbon\Carbon::now()->format("H:i:s")); ?>";
        request('<?php echo e(route('server_stats')); ?>', form, function (response) {
            data = JSON.parse(response);
            <?php if(server()->isLinux()): ?>
                resourceChart('<?php echo e(__("Cpu Kullanımı")); ?>', "cpuChart", data.time, data.cpuPercent);
                resourceChart('<?php echo e(__("Ram Kullanımı")); ?>', "ramChart", data.time, data.ramPercent);
                resourceChart('<?php echo e(__("Disk Kullanımı")); ?>', "diskChart", data.time, data.diskPercent);
                resourceChart('<?php echo e(__("Disk I/O")); ?>', "ioChart", data.time, data.ioPercent);
                networkChart('<?php echo e(__("Network")); ?>', "networkChart", data.time, data.network);
            <?php else: ?>
                if(firstStats){
                    firstStats = false;
    ...
    function getSudoers(){
        ...
        request('<?php echo e(route('server_sudoers_list')); ?>', new FormData(), function (response) {
            Swal.close();
    ...
    function deleteSudoers(row){
        Swal.fire({
            ...
            text: "<?php echo e(__('Yetkili kullanıcıyı silmek istediğinizden emin misiniz?')); ?>",
            type: 'warning',
            ...
            if (result.value) {
                ...
                request('<?php echo e(route('server_delete_sudoers')); ?>',data,function(response){
                    Swal.close();
                    getSudoers();
                }, function(response){
                    var error = JSON.parse(response);
    ...
    function getLocalUsers(){
        ...
        request('<?php echo e(route('server_local_user_list')); ?>', new FormData(), function (response) {
            Swal.close();
    ...
    function getLocalGroups(){
        ...
        request('<?php echo e(route('server_local_group_list')); ?>', new FormData(), function (response) {
            Swal.close();
    ...
    function localGroupDetails(element){
        ...
        request('<?php echo e(route('server_local_group_users_list')); ?>', data, function (response) {
            Swal.close();
    ...
    function addLocalGroupUser(){
        ...
        request('<?php echo e(route('server_add_local_group_user')); ?>',form,function(response){
            var json = JSON.parse(response);
    ...
    function getPackages() {
        showSwal('<?php echo e(__("Okunuyor...")); ?>','info');
        request('<?php echo e(route('server_package_list')); ?>', new FormData(), function (response) {
            Swal.close();
    ...
    function getOpenPorts() {
        showSwal('<?php echo e(__("Okunuyor...")); ?>','info');
        request('<?php echo e(route('server_get_open_ports')); ?>', new FormData(), function (response) {
            var json = JSON.parse(response);
    ...
    function statusService(element) {
        ...
        request('<?php echo e(route('server_service_status')); ?>', form, function (response) {
            var json = JSON.parse(response);
    ...
    function getLogs(page = 1) {
        ...
        request('<?php echo e(route('server_get_logs')); ?>', form, function (response) {
            var json = JSON.parse(response);
    ...
    function getLogDetails(element){
        ...
        request('<?php echo e(route('server_get_log_details')); ?>', form, function (response) {
            var json = JSON.parse(response);
    ...
    function getServices() {
        showSwal('<?php echo e(__("Okunuyor...")); ?>','info');
        request('<?php echo e(route('server_service_list')); ?>', new FormData(), function (response) {
    ...
    function installPackageButton(){
        if($('#installPackage').find('[href="#fromRepo"]').hasClass('active')){
            ...
            if(!packages.length){
                showSwal("<?php echo e(__('Lütfen önce bir deb paketi yükleyin.')); ?>",'error');
                return;
            }
            index = 0;
            installPackage();
    ...
    function onDebUploadSuccess(upload){
        ...
        request('<?php echo e(route('server_upload_deb')); ?>', data, function (response) {
            Swal.close();
    ...
    function installPackage(){
        ...
        request('<?php echo e(route('server_install_package')); ?>', data, function (response) {
            checkPackage();
        }, function(response){
            var error = JSON.parse(response);
            showSwal(error.message,'error',2000);
    ...
    function checkPackage(){
        ...
        request('<?php echo e(route('server_check_package')); ?>', data, function (response) {
            response = JSON.parse(response);
    ...
    function getUpdates(getList) {
        showSwal('<?php echo e(__("Okunuyor...")); ?>','info');
        request('<?php echo e(route('server_update_list')); ?>', new FormData(), function (response) {
    ...
    function removeExtension(){
        ...
        if(data.length === 0){
            showSwal('<?php echo e(__("Lütfen önce seçim yapınız.")); ?>','error',2000);
            return false;
        };
    ...
    function removeExtensionFunc() {
      ...
      showSwal('<?php echo e(__("Siliniyor...")); ?>','info');
      var form = new FormData();
      form.append('extensions',JSON.stringify(data));
      request('<?php echo e(route('server_extension_remove')); ?>', form, function (response) {
          var json = JSON.parse(response);
...
</script><?php /**PATH /liman/server/resources/views/server/one/general/scripts.blade.php ENDPATH**/ ?>